...
public class DepositCauldronItems extends Leaf {
     ItemLooterConfig c = ItemLooterConfig.getInstance();

    @Override
    public boolean isValid() {
        return BankLocation.VARROCK_WEST.getArea(1).contains(Players.getLocal()) && !Inventory.isEmpty();
    }
        ...
        if (Bank.isOpen()) {
            ...
            if (!Inventory.isEmpty()) {
                if (Bank.depositAllItems()) {
                    Sleep.sleepUntil( Inventory::isEmpty, 3000, 50);
                    Bank.close();
                }
            }
        }
